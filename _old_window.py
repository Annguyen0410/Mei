...
class SearchWindow(QMainWindow):
    def __init__(self, base_dir=None, start_tabs=None, app_dir=None, embedded=False, window_slot=None):
        ...
        self.btn_collapse_sidebar.setText("ΓÇ╣")
        ...
        self.btn_panel_tabs.setText("Γùë")
        ...
        self.btn_panel_bookmarks.setText("Γÿà")
        ...
        self.btn_panel_history.setText("Γù╖")
        ...
        self.btn_panel_downloads.setText("Γåô")
        ...
        self.btn_panel_reading.setText("Γÿ░")
        ...
        self.btn_options = QPushButton("ΓÜÖ  Control")
        ...
        self.btn_toggle_topbar.setText("Γû╛")
        ...
        # Android bridge ΓåÆ real tabs: MeiRemote can ask the desktop to open a web
    ...
    def _build_page_menu(self):
        ...
        menu.addSeparator()
        self.act_text_highlight = menu.addAction("Γ£Ä Highlight text to copy")
    ...
    def _refresh_adblock_now(self):
        ...
        def _done(future):
            ...
            QMessageBox.information(
                self,
                "Adblock",
                "─É├ú cß║¡p nhß║¡t bß╗Ö lß╗ìc quß║úng c├ío." if ok else "Ch╞░a c├│ danh s├ích ─æ─âng k├╜ hoß║╖c cß║¡p nhß║¡t thß║Ñt bß║íi.",
            )
    ...
    def show_performance_dashboard(self):
        dlg = QDialog(self)
        dlg.setWindowTitle("Bß║úng ─æo hiß╗çu n─âng ΓÇö Mei")
        dlg.resize(440, 260)
        layout = QVBoxLayout(dlg)
        title = QLabel("Bß║úng ─æo hiß╗çu n─âng")
        title.setObjectName("SectionTitle")
        layout.addWidget(title)
        lbl_ram = QLabel("")
        lbl_tabs = QLabel("")
        lbl_heap = QLabel("JS heap tab hiß╗çn tß║íi: ─æang ─æoΓÇª")
        ...
        def _on_heap(value):
            try:
                val = float(value)
                if val >= 0:
                    lbl_heap.setText(f"JS heap tab hiß╗çn tß║íi: {val:.1f} MB")
                else:
                    lbl_heap.setText("JS heap tab hiß╗çn tß║íi: kh├┤ng khß║ú dß╗Ñng")
            except Exception:
                lbl_heap.setText("JS heap tab hiß╗çn tß║íi: kh├┤ng khß║ú dß╗Ñng")
        ...
        def refresh():
            ...
            lbl_ram.setText(
                f"RAM tiß║┐n tr├¼nh: {rss} MB (Memory Saver k├¡ch hoß║ít > {MEMORY_SAVER_RSS_THRESHOLD_MB} MB)"
                if rss is not None
                else "RAM tiß║┐n tr├¼nh: kh├┤ng ─æß╗ìc ─æ╞░ß╗úc tr├¬n hß╗ç ─æiß╗üu h├ánh n├áy"
            )
            lbl_tabs.setText(f"Tab: {live} live ┬╖ {sleeping} sleeping")
            lbl_heap.setText("JS heap tab hiß╗çn tß║íi: ─æang ─æoΓÇª")
            current = self.current_browser()
            if current is not None:
        ...
        refresh()
        row = QHBoxLayout()
        btn_refresh = QPushButton("─Éo lß║íi")
        btn_refresh.clicked.connect(refresh)
        btn_freeze = QPushButton("Freeze to├án bß╗Ö tab nß╗ün")
    ...
    def _toggle_sidebar_collapse(self):
        ...
        self.btn_collapse_sidebar.setText("ΓÇ║" if self.sidebar_collapsed else "ΓÇ╣")
        self._apply_sidebar_collapse_visibility()
        rail = 30
    ...
    def _apply_topbar_collapse(self):
        ...
        if hasattr(self, "btn_toggle_topbar"):
            self.btn_toggle_topbar.setText("Γû┤" if collapsed else "Γû╛")
    ...
    def open_bundled_site(self, key: str):
        """Open one of the four chained apps (LinkLumina, Cß╗Ñc Quß║ún L├╜, MAS,
    ...
    def show_extension_import_center(self):
        ...
        help_label = QLabel(
            ...
            "then import all tabs into the current workspace ΓÇö or use Import All as Workspaces "
    ...
    def _set_proxy_from_config(self, cfg, prompt_restart: bool = True):
        ...
        if valid:
            ...
            # Never install a half-configured proxy (empty host/port) ΓÇö that
    ...
    def reopen_closed_tab(self):
        ...
        # ``SearchWindow.refresh`` ΓÇö the old call here raised AttributeError.)
